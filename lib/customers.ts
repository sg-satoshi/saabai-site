/**
 * Unified customer directory aggregation.
 *
 * Extracted from app/api/admin/customers/route.ts so the MCP server, the admin
 * routes, and future bots/cron share ONE data-access path (no HTTP loopback).
 * Admin/session auth is NOT performed here — callers enforce access control.
 */
import { getLexClients } from "./lex-config";
import { listSites } from "./site-registry";
import { loadClients } from "./clients";
import { listClients as listLeadGenClients } from "./leadgen-config";
import { listEngagements } from "./audit-store";
import { AUDIT_STATUS_LABELS } from "./audit-types";
...
async function fetchStripeCustomers(): Promise<UnifiedCustomer[]> {
  ...
  try {
    ...
    return subs.data.map((sub) => {
      ...
      const monthlyItem = sub.items.data.find(
...
/**
 * Aggregate every customer source into a single UnifiedCustomer[].
 * Mirrors the previous inline logic in the admin route exactly.
 */
