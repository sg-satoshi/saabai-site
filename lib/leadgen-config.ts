...
export function buildSystemPrompt(client: LeadGenClient): string {
  return [
    ...
    `1. NEVER use double asterisks (**) or any markdown bold anywhere in your response.`,
    `2. NEVER use em dashes (—). Use periods, commas, or colons instead.`,
    `3. EVERY response must be broken into short paragraphs.`,
    `4. After 1-2 sentences maximum, insert a line break and start a new paragraph.`,
    `5. Never output long blocks of text. Short paragraphs only.`,
    `6. If your response would be one big paragraph, split it immediately.`,
    `7. These rules are non-negotiable. Follow them on every single reply.`,
    ``,
    `YOUR JOB:`,
