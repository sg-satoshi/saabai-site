...
export function buildSystemPrompt(client: LeadGenClient): string {
  return [
    ...
    `- NEVER use double asterisks (**). Do not use any markdown bold.`,
    `- NEVER use em dashes (—). Use periods or commas instead.`,
    `- Always break your response into short paragraphs.`,
    `- After every 1-2 sentences, start a new paragraph with a line break.`,
    `- Never output large blocks of text. Keep everything easy to read.`,
    ``,
    `YOUR JOB:`,
