...
function calcStampDuty(price: number, state: State): { duty: number; concession?: string } {
  ...
  switch (state) {
    case "NSW": {
      ...
      else if (price <= 87000) duty = 445 + (price - 32000) * 0.0175;
      else if (price <= 318000) duty = 1407.5 + (price - 87000) * 0.035;
      else if (price <= 1077000) duty = 9692.5 + (price - 318000) * 0.045;
      else if (price <= 3294000) duty = 43857.5 + (price - 1077000) * 0.055;
      else duty = 165792.5 + (price - 3294000) * 0.07;

      // First home buyer exemption (up to $800k, sliding scale)
    ...
    case "VIC": {
      ...
      else if (price <= 2000000) duty = 44370 + (price - 960000) * 0.06;
      else duty = price * 0.065; // Premium rate

      // FHOG eligible for < $750k new homes
      const fhbPossible = price <= 750000;

      return { duty, concession: fhbPossible ? "First home — may qualify for FHOG on new builds" : undefined };
    }
      ...
      <div style={{ maxWidth: 900 }}>
        ...
        <div style={{ marginBottom: 28 }}>
          ...
          <p className="mt-1 text-sm text-[#5C6670]">
            Calculate stamp duty for properties in NSW, VIC, and QLD.
          </p>
        </div>
        ...
        <div className="grid gap-6 md:grid-cols-5">
          ...
          <div className="rounded-2xl border border-[rgba(0,0,0,0.08)] bg-white p-5 md:col-span-2">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="text-xs font-medium text-[#5C6670]">State</label>
                <div className="mt-1 flex gap-2">
                  {(["NSW", "VIC", "QLD"] as State[]).map((s) => (
                    <button
                      key={s}
                      onClick={() => setState(s)}
                      style={{
                        ...
                        background: state === s ? "#0891b2" : "#f3f4f6",
                        color: state === s ? "#fff" : "#5C6670",
          ...
          <div className="md:col-span-3">
            <div className="rounded-2xl border border-[rgba(0,0,0,0.08)] bg-white p-6">
              <p className="text-xs font-semibold uppercase tracking-wider text-[#5C6670] mb-5">
                Results &mdash; {STATE_NAMES[state]}
              </p>
