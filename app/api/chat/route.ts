...
type IncomingMessageRole = "user" | "system" | "assistant";
...
function extractConversationPersistenceFields(
...
This visitor has chatted with Mia before. Their previous conversation is in the message history. Acknowledge them naturally — you remember the conversation. Don't start from scratch or re-introduce yourself fully.`;
  ...
  if (
    visitorProfile &&
    Object.keys(visitorProfile).filter((k) => k !== "updatedAt").length > 0
  ) {
...
function normalizeMessages(messages: IncomingMessage[]): NormalizedIncomingMessage[] {
...
function extractAssistantTextFromFinishEvent(event: unknown): string {
  const response = (event as { response?: unknown } | undefined)?.response as
    | {
        messages?: Array<{
...
export async function POST(req: Request) {
  ...
  const { threadId, projectId } =
    extractConversationPersistenceFields(requestBody as Record<string, unknown>);
  ...
  const result = streamText({
    ...
    onFinish: async (event) => {
      ...
      const hasData =
        Object.keys(sessionData.visitorFacts).length > 0 ||
        sessionData.qualScore !== undefined;

      const assistantText = extractAssistantTextFromFinishEvent(event);

      if (hasData || sessionData.outcome || threadId || projectId || assistantText) {
        const transcript = [
          ...messages.map((m) => ({
        ...
        await saveConversation({
          ...
          keyTopics: Object.keys(sessionData.visitorFacts).filter(
            (k) => sessionData.visitorFacts[k]
          ),
          messages: transcript,
          threadId,
          projectId,
        ...
        if (
          ...
          await saveLead({
            ...
            pain_points: sessionData.visitorFacts.pain_points as
              | string[]
              | undefined,
    ...
    tools: {
      remember_visitor: tool({
        ...
        inputSchema: z.object({
          ...
          business: z
            .string()
            .optional()
            .describe("What their business does, in one sentence"),
          industry: z
            .string()
            .optional()
            .describe(
              "Their industry (e.g. accounting, real estate, law, construction)"
            ),
          team_size: z
            .string()
            .optional()
            .describe("Their team size, e.g. '12 people', 'small team of 3'"),
          pain_points: z
            .array(z.string())
            .optional()
            .describe("Key pain points or challenges they've mentioned"),
        }),
        execute: async (args) => {
      ...
      calculate_roi: tool({
        ...
        inputSchema: z.object({
          team_members: z
            .number()
            .describe("Number of people who touch this process"),
          hours_per_person_per_week: z
            .number()
            .describe("Hours each person spends on this process per week"),
          hourly_rate: z
            .number()
            .describe("Average hourly cost for these team members in AUD"),
          process_name: z
            .string()
            .describe(
              "Short name for the process, e.g. 'client follow-up', 'document chasing', 'reporting'"
            ),
        }),
        execute: async (args) => {
      ...
      qualify_lead: tool({
        ...
        execute: async (args) => {
          const score = [
            args.business_fit,
            args.pain_point_named,
            args.automation_potential,
      ...
      show_booking_cta: tool({
        ...
        execute: async (args) => {
          ...
          if (!sessionData.hotLeadFired) {
            ...
            const flatConversation = messages
      ...
      fetch_webpage: tool({
        ...
        inputSchema: z.object({
          url: z
            .string()
            .describe("The full URL to fetch, exactly as shared by the visitor."),
        }),
        execute: async ({ url }) => {
          try {
            ...
            if (!res.ok) {
              return { error: `Could not load page (HTTP ${res.status})` };
            }
            ...
            return {
              error: "Could not read that page. It may be blocking automated access.",
            };
          }
        },
      }),
      ...
      capture_lead: tool({
        ...
        inputSchema: z.object({
          reason: z
            .string()
            .describe(
              "Why this visitor is going to lead capture instead of booking."
            ),
        }),
        execute: async (args) => {
          sessionData.outcome = "lead_captured";
