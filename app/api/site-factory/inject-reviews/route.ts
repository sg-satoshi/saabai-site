...
function buildCarousel(
  ...
  const stars = (r: number) =>
    [1, 2, 3, 4, 5].map(i => (i <= Math.round(r) ? starFull : starEmpty)).join("");
  ...
  const cards = reviews.map(r => {
    ...
    return (
      ...
      `<div style="display:flex;align-items:center;gap:3px;margin-top:2px;">${stars(r.rating)}` +
      `<span style="margin-left:2px;">${googleG}</span></div></div></div>` +
      `<p style="margin:0;font-size:13px;color:#3c4043;line-height:1.6;max-height:100px;overflow:hidden;">${safe}</p>` +
  ...
  return (
    `\n<section class="sf-reviews-carousel" style="padding:60px 20px;background:#f8f9fa;` +
    ...
    `function tick(){if(!p&&++f%2===0){` +
    `if(t.scrollLeft+t.clientWidth>=t.scrollWidth-4){t.scrollLeft=0}else{t.scrollLeft+=1}}` +
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Remove any previously injected reviews section
    html = html.replace(/<section[^>]+class="sf-reviews-carousel"[\s\S]*?<\/section>/i, "");

    const carousel = buildCarousel(reviews, rating, totalReviews, businessName);

    // Inject before <footer, before </body>, or append
