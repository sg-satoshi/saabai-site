...
- FULL SHEET CAP — critical three-step check:
  STEP 1 — FIT CHECK (try BOTH orientations before concluding it doesn't fit): A piece fits in a sheet if EITHER orientation works — original (W×H) or rotated (H×W). For standard 2440×1220mm: check if (piece_W ≤ 2440 AND piece_H ≤ 1220) OR (piece_H ≤ 2440 AND piece_W ≤ 1220). If either passes, it FITS in the standard sheet. Only escalate to oversized if BOTH orientations fail. Example: 630×1800mm — orientation 1: 630≤2440 but 1800>1220 ✗. Orientation 2 rotated: 1800≤2440 and 630≤1220 ✓ → FITS in standard sheet.
  STEP 2 — PRICE CAP: Compare CTS price (rate × area) against the correct sheet price. If it fits in the standard sheet, cap at standard full sheet price. If it requires an oversized sheet (both orientations fail), find the smallest oversized sheet it fits in (same rotation logic) and cap at that price.
  STEP 3 — CUTTING FEE: Only add $30 cutting fee when the cap actually triggers (CTS price > sheet price). If CTS price ≤ sheet price, quote CTS — no cutting fee unless the total is under $50.
  Example A: 6mm clear 630×1800mm. Rotated fits standard 2440×1220. CTS = 0.630×1.800×$177.77 = $201.59 < $252 → quote $201.59 Ex GST. No cap, no cutting fee.
  Example B: 6mm clear 1915×1800mm. Both orientations fail vs 2440×1220 (1800>1220 and 1915>1220). Oversized 2490×1880: 1915≤2490 and 1800≤1880 ✓. CTS = $612.90 > $402 → $402 + $30 cutting fee = $432 Ex GST.
