...
const IMAGE_TYPES = [
  { value: "ai",          label: "AI Generate",    hint: "Grok image generation" },
...
function CaptionGenerator({ onQueued }: { onQueued: () => void }) {
  ...
  function buildImageUrl(): string | null {
    if (imageType === "ai")     return aiImageUrl;
            ...
            {imageType === "ai" ? (
              <div>
                <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                  <button
                    onClick={generatingImage ? undefined : (aiImageUrl ? () => { setAiImageUrl(null); setImageGenError(""); } : generateAiImage)}
                    disabled={generatingImage || !topic.trim()}
                    style={{
                      padding: "9px 18px", borderRadius: 8, border: "1px solid",
                      borderColor: aiImageUrl ? "#fca5a5" : "#dc2743",
                      background: aiImageUrl ? "#fff5f5" : "#fff0f3",
                      color: aiImageUrl ? "#ef4444" : "#dc2743",
                      fontSize: 12, fontWeight: 700,
                      cursor: (generatingImage || !topic.trim()) ? "not-allowed" : "pointer",
                      opacity: (generatingImage || !topic.trim()) ? 0.6 : 1,
                    }}
                  >
                    {generatingImage ? "Generating…" : aiImageUrl ? "✕ Remove & regenerate" : "Generate image with Grok"}
                  </button>
                  {!topic.trim() && <span style={{ fontSize: 11, color: "#9ca3af" }}>Select a topic first</span>}
                </div>
                {imageGenError && <p style={{ margin: "8px 0 0", fontSize: 11, color: "#ef4444" }}>{imageGenError}</p>}
                {aiImageUrl && (
                  <div style={{ marginTop: 10 }}>
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={aiImageUrl} alt="AI generated" style={{ width: "100%", borderRadius: 8, border: "1px solid #e5e7eb" }} />
                    <p style={{ margin: "6px 0 0", fontSize: 10, color: "#9ca3af" }}>AI-generated by Grok · will be posted to Instagram</p>
                  </div>
                )}
              </div>
            ) : imageType === "custom" ? (
